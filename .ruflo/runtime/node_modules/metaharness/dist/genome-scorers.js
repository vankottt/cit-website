 ...
 */
export function classifyRepoType(p) {
    const parts = [];
    ...
    if (langs.includes('rust') && langs.includes('typescript'))
        parts.push('polyglot');
    return parts.join('_');
}
...
export function scoreMcpRisk(profile, plan) {
    if (plan.mcp === 'off')
        return { surface: 'off', numeric: 0 };
    if (plan.mcp === 'remote')
        return { surface: 'remote', numeric: 0.6 };
    // plan.mcp === 'local'
    const p = plan.policy;
    const permissive = p.allowShell || p.allowNetwork || p.allowFileWrite || !p.defaultDeny;
    if (permissive)
        return { surface: 'local_permissive', numeric: 0.45 };
 ...
 */
export function scoreTestConfidence(profile) {
    let s = 0;
    if (profile.testCommands.length > 0)
        s += 0.5;
    if (profile.testCommands.length >= 2)
        s += 0.2;
    if (profile.hasCi)
        s += 0.3;
    return Math.min(1, s);
 ...
 */
export function scorePublishReadiness(profile, plan) {
    let s = 0;
    if (profile.languages.includes('typescript'))
        s += 0.3; // has package.json
    if (profile.languages.includes('rust'))
        s += 0.15; // has Cargo.toml
    if (profile.buildCommands.length > 0)
        s += 0.2; // can build
    if (profile.testCommands.length > 0)
        s += 0.15; // can test
    if (profile.hasCi)
        s += 0.2; // ci will gate it
    if (plan.policy.defaultDeny)
        s += 0.05; // policy locked
    return Math.min(1, s);
}
//# sourceMappingURL=genome-scorers.js.map