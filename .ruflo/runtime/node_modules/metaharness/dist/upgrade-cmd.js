 ...
 */
export async function upgradeCmd(args) {
    const positional = args.filter(a => !a.startsWith('--'));
    const dir = resolve(positional[0] ?? process.cwd());
    const apply = args.includes('--apply');
    const conflictStyle = args.find(a => a.startsWith('--conflict='))?.slice('--conflict='.length) ?? 'inline';
    const lines = [`harness upgrade — ${dir} ${apply ? '(APPLY)' : '(DRY-RUN)'}`];
