...
export async function verify(args) {
    ...
    if (!wp) {
        pushLines(lines, `No witness.json found under ${dir}.`, `Looked at: ${dir}/witness.json and ${dir}/.harness/witness.json`, `Run 'harness sign' first to produce one.`);
        return { code: 1, lines };
    }
    try {
...
export async function doctor(args) {
    ...
    if (existsSync(pkgPath)) {
        try {
            ...
            check(!!(pkg.dependencies && pkg.dependencies['@metaharness/kernel']), 'declares @metaharness/kernel as dependency');
        }
        catch {
    ...
    check(hasClaudeCode || hasCodex || hasPi || hasHermes, 'at least one host artifact present (.claude/, .codex/, AGENTS.md, or cli-config.yaml)');
    ...
    pushLines(lines, '', `Next: capture the full diagnostic state for a support ticket:`, `  harness diag ${dir} --bundle > bundle.json`, `(then attach bundle.json to a GitHub issue at`, ` https://github.com/ruvnet/agent-harness-generator/issues — the`, ` bundle is sanitised; secret_/token_/key_/password_ fields are redacted)`);
    return { code: 1, lines };
}
/**
...
export async function sign(args) {
    ...
    if (!keyHex) {
        pushLines(lines, `WITNESS_SIGNING_KEY env var not set.`, `In CI: fetch from GCP Secret Manager via WIF (see docs/setup/gcp-secrets.md).`, `Locally: export WITNESS_SIGNING_KEY=<64-hex-char string>`);
        return { code: 1, lines };
    }
    ...
    try {
        ...
        const entries = Object.entries(manifest.files).map(([path, sha256]) => ({
            id: path,
        ...
        let signedManifest;
        try {
            const kernel = await import('@metaharness/kernel');
 ...
 */
export async function dispatch(subcommand, args) {
