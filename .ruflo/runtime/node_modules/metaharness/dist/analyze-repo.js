...
export function inventory(dir) {
    ...
    if (existsSync(join(root, '.github', 'workflows')))
        files['.github/workflows/ci.yml'] = '# present';
...
export function analyzeFiles(name, files) {
    const get = (p) => files[p] ?? '';
    const languages = [];
    if (get('Cargo.toml'))
        languages.push('rust');
    ...
    if (get('pyproject.toml') || get('requirements.txt'))
...
function eligible(profile, a) {
    if (a.requiredLang && !profile.languages.includes(a.requiredLang))
        return false;
    if (a.requiredSignal === 'hasMcp' && !profile.hasMcp)
        return false;
    return true;
}
export function scoreArchetypes(profile, semantic) {
...
export function recommendPlan(profile, semantic) {
    ...
    return {
        ...
        suggestedCommands: [...profile.buildCommands, ...profile.testCommands].map((command) => ({ command, trust: 'inferred', execution: 'disabled' })),
    };
}
 ...
 */
const ruvllmCache = new Map();
function ruvllmCacheKey(profile) {
...
export function ruvllmSemantic(profile) {
    ...
    try {
        ...
        const q = Array.from(llm.embed(qText));
        const out = {};
        for (const a of ARCHETYPES) {
            const v = Array.from(llm.embed(`${a.label}. ${a.description} ${a.keywords.join(' ')}`));
...
export function _resetRuvllmCacheForTests() {
    ruvllmCache.clear();
}
// --- CLI command -----------------------------------------------------------
export async function analyzeRepoCmd(args) {
    let dir = '.';
    let embed = false;
    let outDir = null;
    let json = false;
    let scaffoldName = null;
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--embed')
            ...
            scaffoldName = args[++i] ?? null;
        else if (a === '--host')
            host = (args[++i] ?? 'claude-code');
        else if (a && !a.startsWith('-'))
            dir = a;
    }
    const root = resolve(dir);
    const lines = [`harness analyze-repo — ${root}`, ''];
    ...
    if (embed && !semantic)
        lines.push('  (note: --embed requested but @ruvector/ruvllm unavailable — used lexical)');
    ...
    if (plan.suggestedCommands.length) {
        ...
        for (const c of plan.suggestedCommands)
            lines.push(`  - ${c.command}  [trust: ${c.trust}]`);
    }
    lines.push('');
