...
import { inventory, analyzeFiles, recommendPlan, scoreArchetypes, ruvllmSemantic } from './analyze-repo.js';
 ...
 */
export function buildRepoScorecard(dir, generatedAt = new Date().toISOString()) {
    ...
    try {
        const pkgName = files['package.json'] ? JSON.parse(files['package.json']).name : undefined;
    ...
    const compileConfidence = clamp100((profile.languages.length ? 40 : 12) +
    ...
    if (!p.allowFileWrite)
        safety += 10;
    if (p.requireApprovalForDangerous)
        safety += 15;
    if (p.auditLog)
        safety += 15;
    if (plan.mcp === 'remote')
        safety -= 10; // remote MCP is more attack surface
    ...
    const memoryUsefulness = clamp100(Math.min(fileCount, 30) * 2 + profile.languages.length * 5 + Math.min(profile.tokens.length, 25));
    ...
    const recommendedMode = plan.mcp === 'off' ? 'CLI' : 'CLI + MCP';
...
export function formatRepoScorecard(sc) {
    return [
 ...
 */
export function topCandidates(dir, n = 3) {
    const files = inventory(dir);
    let name = basename(resolve(dir)) || 'repo';
    try {
        const pkgName = files['package.json'] ? JSON.parse(files['package.json']).name : undefined;
    ...
    return scoreArchetypes(profile, ruvllmSemantic(profile))
        ...
        recommendedMode: (s.archetype.mcp === 'off' ? 'CLI' : 'CLI + MCP'),
    }));
}
/** Full constraint report for a repo's recommended design (ADR-041 validation). */
export function repoConstraints(dir) {
    const files = inventory(dir);
    let name = basename(resolve(dir)) || 'repo';
    try {
        const pkgName = files['package.json'] ? JSON.parse(files['package.json']).name : undefined;
...
export function formatCandidates(repo, cands) {
    return [
...
export async function scoreRepoCmd(args) {
    const json = args.includes('--json');
    const positional = [];
    let topN = null;
    for (let i = 0; i < args.length; i++) {
        const a = args[i];
        if (a === '--json' || a === '--constraints')
            continue;
        if (a === '--help' || a === '-h')
        ...
        if (a === '--top') {
            ...
            if (!Number.isFinite(v) || v < 1) {
                const err = { schema: 1, error: 'invalid-top', exitCode: 2 };
    ...
    try {
        ...
        if (topN != null) {
            ...
            const repo = (() => {
                try {
                    ...
                    return (f && JSON.parse(f).name) || basename(dir);
                }
                catch {
                    return basename(dir);
                }
        ...
        const e = { schema: 1, error: 'score-failed', detail: msg, exitCode: 1 };
        return { code: 1, lines: [json ? JSON.stringify(e, null, 2) : `score failed: ${msg}`] };
    }
}
//# sourceMappingURL=repo-scorecard.js.map