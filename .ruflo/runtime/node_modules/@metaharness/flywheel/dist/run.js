...
export async function runFlywheelGenerations(cfg) {
    ...
    const evalCache = cfg.cacheEvaluations ? new Map() : null;
    const evaluate = async (p, suite) => {
        if (!evalCache)
            return cfg.evaluator(p, suite);
    ...
    const anchorOf = async (p) => cfg.anchor ? (await evaluate(p, cfg.anchor)).primary : null;
    let rootScore;
    let rootAnchor;
    let parentId;
    let policy;
    ...
    for (let gen = startGen; gen <= cfg.maxGenerations; gen++) {
        ...
        for (const target of targets) {
            ...
            const candPolicy = { ...policy, [target]: norm.value };
        ...
        for (const c of cands) {
            ...
            const verdict = isWinner ? 'PROMOTED' : 'REJECTED';
