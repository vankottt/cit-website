 ...
 *  the check. */
function receiptMatchesCommit(c) {
    const p = c.receipt.payload;
    if (c.verdict === 'ROOT')
        return p.kind === 'root' && p.root === c.id;
...
export function verifyReplayBundle(bundle, opts = {}) {
    ...
    if (!allCommitsReceipts)
        failures.push('allCommitsReceipts');
    ...
    let sealedFieldsAuthentic = true;
    {
        const seen = new Set();
        for (const c of [...chain, ...bundle.all_commits]) {
            if (seen.has(c.id) || c.verdict === 'ROOT')
                continue;
            seen.add(c.id);
            const payload = c.receipt.payload;
            for (const [k, live] of Object.entries(BOUND_KEYS)) {
                if (!(k in payload))
                    continue;
                if (canon(payload[k]) !== canon(live(c))) {
                    sealedFieldsAuthentic = false;
                    break;
                }
            }
            if (!sealedFieldsAuthentic)
                break;
        }
    }
    if (!sealedFieldsAuthentic)
        failures.push('sealedFieldsAuthentic');
    return {
