#!/usr/bin/env node
import { a as startHTTPServer, c as getUpstreamBridge, d as __commonJSMin, f as __toESM, i as DEFAULT_ALLOWED_HEADERS, n as resolveVersionNegotiation, o as proxyServer, s as acquireListenSubscriptions } from "../startStdioServer-BIyBMtB8.mjs";
import { createRequire } from "node:module";
import { Server } from "@modelcontextprotocol/server";
import { basename, dirname, extname, join, normalize, relative, resolve } from "path";
import { format, inspect } from "util";
import { readFileSync, readdirSync, statSync, writeFile } from "fs";
import { Client, ReadBuffer, STDIO_DEFAULT_MAX_BUFFER_SIZE, serializeMessage } from "@modelcontextprotocol/client";
...
//#endregion
//#region src/createGracefulShutdown.ts
...
var JSONFilterTransform = class extends Transform {
	...
	_transform(chunk, _encoding, callback) {
		...
		if (this.buffer.length > MAX_BUFFER_SIZE) {
			this.buffer = "";
			callback(/* @__PURE__ */ new Error(`JSONFilterTransform buffer exceeded maximum size of ${MAX_BUFFER_SIZE} bytes`), null);
			return;
		}
		const jsonLines = [];
		const nonJsonLines = [];
	...
	*/
	...
	send(message) {
		return new Promise((resolve$1, reject) => {
			...
			const onClose = () => settle(/* @__PURE__ */ new Error("Stdin closed before the message was sent"));
			const onError = (error) => settle(error);
			const onExit = () => settle(/* @__PURE__ */ new Error("Child process exited before the message was sent"));
...
const argv = await yargs_default(hideBin(process.argv)).scriptName("mcp-proxy").version(packageJson.version).command("$0 [command] [args...]", "Proxy an MCP stdio server over HTTP").positional("command", {
	...
	maxBodySize: {
		default: 10485760,
		describe: "Maximum request body size in bytes accepted by the stream endpoint; larger requests are answered with 413 Payload Too Large. Bounds the memory a single request can consume (default: 10 MiB). Set to 0 to disable the limit",
		type: "number"
	},
	modern: {
		default: true,
		describe: "Serve MCP protocol revision 2026-07-28 on the stream endpoint alongside the 2025-era revisions. Each request is classified by its own content, so one endpoint serves both and neither client needs to know the other exists. Use --no-modern to serve only 2025-era clients. Has no effect with --server sse, which disables the stream endpoint the leg lives on",
		type: "boolean"
	},
	port: {
		default: 8080,
	...
	upstreamProtocol: {
		...
		default: "legacy",
		describe: "Which MCP protocol revision to speak to the spawned server. 'legacy' (default) uses the 2025 initialize handshake. 'auto' sends server/discover first and falls back to the handshake — needed to front a 2026-07-28 server, at the cost of one extra round trip, and of sending that probe to a server that may not expect it. 'modern' requires 2026-07-28 and fails against an older server",
		type: "string"
	}
}).help().parseAsync();
...
if (!(argv.maxBodySize >= 0)) {
	console.error(`Error: --maxBodySize must be a number >= 0 (got ${String(argv.maxBodySize)}). Use --maxBodySize 0 to disable the limit instead.`);
	process.exit(1);
}
if (!(argv.sessionIdleTimeout >= 0)) {
	console.error(`Error: --sessionIdleTimeout must be a number >= 0 (got ${String(argv.sessionIdleTimeout)}). Use --sessionIdleTimeout 0 to keep sessions until their client sends DELETE instead.`);
	process.exit(1);
}
const corsOption = argv.corsAddAllowedHeader && argv.corsAddAllowedHeader.length > 0 ? { allowedHeaders: [...DEFAULT_ALLOWED_HEADERS, ...argv.corsAddAllowedHeader] } : void 0;
