...
class GNNServiceStub {
    constructor(config) { }
    async initialize() { }
    async classifyIntent(embedding) {
        return { intent: 'unknown', confidence: 0 };
    }
    async classifyNode(embedding) {
        return { category: 'unknown', confidence: 0 };
    }
    async predictLink(nodeA, nodeB) {
        return 0.5;
    }
    async predictLinks(nodes) {
        return nodes.map(() => nodes.map(() => 0.5));
    }
    async getNodeEmbedding(nodeId) {
        return new Float32Array(768);
    }
    async understandContextGAT(embedding, contextNodes) {
        return {
            attentionWeights: {},
            aggregatedContext: new Float32Array(embedding.length)
        };
    }
    async processHeterogeneousGraph(graph) {
        return { nodes: [], edges: [] };
    }
    getEngineType() {
        return 'stub';
    }
    async matchSkillsGCN(taskEmbedding, skillNodes) {
...
export class GNNRouterService {
     ...
     * - Task context (GAT)
     */
    async routeTask(task, availableAgents, contextNodes) {
        ...
        for (const agentId of availableAgents) {
            ...
            for (const skillMatch of skillMatches) {
                if (agent.skills.includes(skillMatch.skill)) {
                    ...
                    reasons.push(`Has skill "${skillMatch.skill}" (${(skillMatch.score * 100).toFixed(1)}% match)`);
                }
            }
            ...
            if (contextNodes && contextNodes.length > 0) {
                const contextResult = await this.gnnService.understandContextGAT(agent.embedding, contextNodes);
                const contextScore = Object.values(contextResult.attentionWeights).reduce((sum, w) => sum + w, 0) / contextNodes.length;
     ...
     */
    async classifyTask(task, relatedTasks) {
        ...
        const neighborEmbeddings = relatedTasks
            ...
            : [];
        const result = await this.gnnService.classifyNode(task.embedding, neighborEmbeddings, categories);
     ...
     */
    async optimizeWorkflow(tasks, existingDependencies) {
        const suggestions = [];
        ...
        for (const sourceTask of tasks) {
            ...
            const predictions = await this.gnnService.predictLinks({ id: sourceTask.id, embedding: sourceTask.embedding }, candidateNodes, existingDependencies, 5);
     ...
     */
    async analyzeSystemGraph(queryType, queryId) {
        ...
        const result = await this.gnnService.processHeterogeneousGraph({ nodes, edges }, queryNodeId);
        // Generate insights
        const insights = [];
        ...
        if (agentNodes.length > 0) {
            insights.push(`Most relevant agents: ${agentNodes.slice(0, 3).map(n => n.id).join(', ')}`);
        }
        if (skillNodes.length > 0) {
            insights.push(`Key skills: ${skillNodes.slice(0, 3).map(n => n.id).join(', ')}`);
        }
        if (taskNodes.length > 0) {
            insights.push(`Related tasks: ${taskNodes.slice(0, 3).map(n => n.id).join(', ')}`);
        }
        if (result.pathways.length > 0) {
            insights.push(`Strongest pathway: ${result.pathways[0].path.join(' → ')} (strength: ${result.pathways[0].strength.toFixed(2)})`);
        }
        return {
            ...result,
            insights,
     ...
     */
    recordTaskExecution(taskId, agentId, success, duration) {
    ...
    // Private methods
    // ---------------------------------------------------------------------------
    async matchTaskToSkills(task) {
        const skillGraphFormatted = {};
        for (const [skillName, skillNode] of Object.entries(this.skillGraph)) {
    ...
    estimateSuccessRate(agentId, skillMatches) {
        const agent = this.agentProfiles.get(agentId);
        if (!agent)
            return 0.5;
...
//# sourceMappingURL=gnn-router-service.js.map