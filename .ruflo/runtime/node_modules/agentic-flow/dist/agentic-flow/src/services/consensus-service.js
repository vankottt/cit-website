...
class RaftConsensusStub extends EventEmitter {
    start() { }
    stop() { }
    async replicate(command, timeout) { return true; }
    getStatus() {
        return {
            nodeId: 'node-1',
            state: 'follower',
    ...
    getCRDT(key) { return null; }
    async acquireLock(key, ttlMs) { return true; }
    async releaseLock(key) { }
    detectDeadlocks() { return []; }
}
...
export class ConsensusService extends EventEmitter {
     ...
     */
    async replicateCommand(command, timeout) {
        if (!this.raft) {
     ...
     */
    async createPartition(partitionId, nodes) {
     ...
     */
    async resolveDeadlocks(deadlocks) {
