...
import { RuVectorLearning } from '../backends/ruvector/RuVectorLearning.js';
...
export class LearningSystem {
     ...
     */
    async initializeRuVectorEnhancements() {
    ...
    `).run(feedback.sessionId, feedback.state, feedback.action, feedback.reward, feedback.nextState || null, feedback.success ? 1 : 0, feedback.timestamp);
        ...
        if (this.sonaEnabled && this.sonaService) {
            try {
                ...
                await this.sonaService.recordTrajectory(session.sessionType, [trajectoryStep]);
            }
            catch (error) {
            ...
            try {
                ...
                for (const [key, qValue] of Object.entries(policy.qValues)) {
                    ...
                    if (similarStates.length >= 5)
                        break; // Limit to top 5 neighbors
                }
                if (similarStates.length > 0) {
                    // Enhance state embedding with GNN
                    const enhancedEmbedding = this.gnnLearning.enhance(stateEmbedding, similarStates, weights);
