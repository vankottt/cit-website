...
import { AttentionService } from '../utils/LegacyAttentionAdapter.js';
export class CausalMemoryGraph {
    db;
    graphBackend; // GraphBackend or GraphDatabaseAdapter
    attentionService;
    embedder;
    config;
    /**
     ...
     */
    constructor(db, graphBackend, embedder, config) {
        this.db = db;
     ...
     */
    async getCausalChain(fromMemoryId, toMemoryId, maxDepth = 5) {
     ...
     * @private
     */
    async getCausalChainWithAttention(fromMemoryId, toMemoryId, maxDepth) {
    ...
    `).all(fromMemoryId, maxDepth, toMemoryId);
        ...
        for (const nodeId of allNodeIds) {
            const episode = this.db.prepare('SELECT task, output FROM episodes WHERE id = ?').get(nodeId);
            if (episode) {
        ...
        const rankedChains = candidateChains
            ...
            const avgWeight = path.reduce((sum, nodeId) => {
