...
export class ReflexionMemory {
    ...
    learningBackend;
    graphBackend;
    constructor(db, embedder, vectorBackend, learningBackend, graphBackend) {
        this.db = db;
    ...
    async retrieveRelevant(query) {
        ...
        if (this.graphBackend && 'searchSimilarEpisodes' in this.graphBackend) {
            ...
            const filtered = results.filter((ep) => {
                if (minReward !== undefined && ep.reward < minReward)
                    return false;
                if (onlyFailures && ep.success)
                    return false;
                if (onlySuccesses && !ep.success)
                    return false;
                if (timeWindowDays && ep.createdAt < (Date.now() - timeWindowDays * 86400000))
                    return false;
                return true;
            });
            // Convert to EpisodeWithEmbedding format
            const episodes = filtered.slice(0, k).map((ep) => ({
      ...
      `);
                ...
                for (const result of searchResults) {
                    ...
                    if (minReward !== undefined && row.reward < minReward)
                        continue;
                    if (onlyFailures && row.success === 1)
                        continue;
                    if (onlySuccesses && row.success === 0)
                        continue;
                    if (timeWindowDays && row.ts < (Date.now() / 1000 - timeWindowDays * 86400))
                        continue;
                    episodes.push({
                        id: row.id,
                        ts: row.ts,
    ...
    async createEpisodeGraphNode(episodeId, episode, embedding) {
        ...
        await this.graphBackend.createRelationship(nodeId, sessionNodeId, 'BELONGS_TO_SESSION', { timestamp: episode.ts || Date.now() });
