...
export class ReasoningBank {
    ...
    async storePattern(pattern) {
        ...
        const embedding = await this.embedder.embed(`${pattern.taskType}: ${pattern.approach}`);
    ...
    `);
        const result = stmt.run(pattern.taskType, pattern.approach, pattern.successRate, pattern.uses || 0, pattern.avgReward || 0.0, pattern.tags ? JSON.stringify(pattern.tags) : null, pattern.metadata ? JSON.stringify(pattern.metadata) : null);
        ...
        try {
            ...
            if (query.useGNN && this.learningBackend) {
                ...
                if (candidates.length > 0) {
                    ...
                    const neighborEmbeddings = await this.getEmbeddingsForVectorIds(candidates.map(c => c.id));
     ...
     */
    async searchPatternsLegacy(query) {
        const k = query.k || 10;
        const threshold = query.threshold || 0.0;
     ...
     */
    async getEmbeddingsForVectorIds(vectorIds) {
    ...
    getPatternStats() {
        ...
        // Total patterns
        const totalRow = this.db.prepare('SELECT COUNT(*) as count FROM reasoning_patterns').get();
     ...
     */
    updatePatternStats(patternId, success, reward) {
        const stmt = this.db.prepare(`
     ...
     */
    async recordOutcome(patternId, success, reward) {
