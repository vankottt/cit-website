...
export class SkillLibrary {
    ...
    constructor(db, embedder, vectorBackend, graphBackend) {
        this.db = db;
    ...
    `);
        ...
        const result = stmt.run(skill.name, skill.description || null, JSON.stringify(signature), skill.code || null, skill.successRate, uses, avgReward, avgLatencyMs, skill.metadata ? JSON.stringify(skill.metadata) : null);
     ...
     */
    async retrieveSkillsLegacy(query) {
        // v1 API compatibility: accept both 'query' and 'task'
        const task = query.task || query.query;
        if (!task) {
            throw new Error('SkillQuery must provide either task (v2) or query (v1)');
     ...
     */
    storeSkillEmbeddingLegacy(skillId, embedding) {
        const stmt = this.db.prepare(`
