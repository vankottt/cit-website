 ...
 * See ADR-073 §C.1.
 */
export function parseJsonStrict(json, fieldName) {
 ...
 */
export function validateSqlIdentifier(name) {
    ...
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(trimmed)) {
        throw new ValidationError(`SQL identifier contains invalid characters: ${trimmed}`, 'INVALID_IDENTIFIER', 'identifier');
    }
    if (trimmed.length > 128) {
        throw new ValidationError('SQL identifier exceeds maximum length of 128 characters', 'INVALID_IDENTIFIER', 'identifier');
    }
    return trimmed;
}
/**
