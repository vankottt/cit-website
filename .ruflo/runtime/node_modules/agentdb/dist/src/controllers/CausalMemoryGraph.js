...
export class CausalMemoryGraph {
     ...
     */
    constructor(db, graphBackend, embedder, config, vectorBackend) {
        this.db = db;
     ...
     */
    async findSimilarCausalPatterns(mechanism, k = 10, minConfidence = 0.5) {
        ...
        const filteredResults = results.filter(result => {
            if (!result.id.startsWith('causal-edge:'))
                return false;
        ...
        for (const result of filteredResults.slice(0, k)) {
            ...
            if (isNaN(edgeId))
                continue;
            const row = this.db.prepare('SELECT * FROM causal_edges WHERE id = ?').get(edgeId);
            if (row) {
                edges.push({
