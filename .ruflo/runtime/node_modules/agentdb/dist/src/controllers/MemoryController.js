 ...
 */
import { assertEmbeddingSpaceCompatible } from '../embedding/EmbeddingSpaceIdentity.js';
...
export class MemoryController {
    ...
    async update(id, updates) {
        ...
        this.assertMutationIdentity(updates.embeddingSpaceIdentity ?? existing.embeddingSpaceIdentity);
        const updated = {
            ...existing,
            ...updates,
    ...
    async search(query, options = {}) {
        if (this.config.embeddingSpaceIdentity && options.embeddingSpaceIdentity) {
            assertEmbeddingSpaceCompatible(this.config.embeddingSpaceIdentity, options.embeddingSpaceIdentity, 'vector-read');
        }
    ...
    assertMutationIdentity(identity) {
        const expected = this.config.embeddingSpaceIdentity;
        if (!expected)
            return;
        if (!identity) {
            throw new Error('Memory mutation requires embeddingSpaceIdentity when the controller has an authoritative space');
        }
