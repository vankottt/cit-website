...
export class SkillLibrary {
    ...
    queryCache;
    constructor(db, embedder, vectorBackend, graphBackend, cacheConfig) {
        this.db = db;
    ...
    async retrieveSkills(query) {
        ...
        // Check cache first
        const cacheKey = this.queryCache.generateKey('retrieveSkills', [task, k, minSuccessRate, preferRecent], 'skills');
        ...
        if (this.vectorBackend) {
            ...
            for (const result of searchResults) {
                ...
                skillsWithSimilarity.push({
                    ...
                    signature: safeJsonParse(row.signature, undefined),
    ...
    analyzeLearningTrend(episodes) {
        ...
        const firstHalfReward = sorted.slice(0, Math.floor(sorted.length / 2)).reduce((sum, ep) => sum + ep.reward, 0) /
            Math.floor(sorted.length / 2);
        const secondHalfReward = sorted.slice(Math.floor(sorted.length / 2)).reduce((sum, ep) => sum + ep.reward, 0) /
