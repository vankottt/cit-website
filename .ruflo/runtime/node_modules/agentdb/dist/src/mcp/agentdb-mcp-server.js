...
import { buildMcpToolPolicy, filterMcpTools, isMcpToolAllowed, } from '../security/mcp-policy.js';
        ...
        switch (name) {
          ...
          `).get();
            ...
            case 'causal_traverse': {
                const fromId = args?.from_memory_id;
                const toId = args?.to_memory_id;
                const maxDepth = args?.max_depth || 5;
                const chain = await causalGraph.getCausalChain(fromId, toId, maxDepth);
                const found = chain && Array.isArray(chain.edges) ? chain.edges.length > 0 : false;
                return {
                    content: [{
                            type: 'text',
                            text: found
            ...
            case 'agentdb_delete_batch': {
                const ids = args?.ids || [];
                const requestedTable = args?.table || 'episodes';
