 ...
 */
async function executeHyperbolicOp(options, wasmAvailable) {
 ...
 */
async function expMap(options, curvature) {
 ...
 */
async function logMap(options, curvature) {
 ...
 */
async function mobiusAdd(options, curvature) {
 ...
 * Poincaré distance
 */
async function poincareDistance(options, curvature) {
    const x = parseVector(options.pointA, 'point-a');
    const y = parseVector(options.pointB, 'point-b');
    if (x.length !== y.length) {
        throw new Error('Points must have same dimension');
    ...
    const euclideanDist = Math.sqrt(x.reduce((sum, xi, i) => sum + Math.pow(xi - y[i], 2), 0));
    return {
        distance,
 ...
 */
async function projectToBall(options, curvature) {
    const point = parseVector(options.point, 'point');
 ...
 * Hyperbolic centroid
 */
async function hyperbolicCentroid(options, curvature) {
    if (!options.points) {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        // Average gradient
        const avgGradient = gradients[0].map((_, i) => gradients.reduce((sum, g) => sum + g[i], 0) / gradients.length);
    ...
    const avgDistance = points.reduce((sum, point) => sum + poincareDistanceValue(point, centroid, curvature), 0) / points.length;
    return {
        result: centroid,
 ...
 */
async function dualSpaceSearch(options, curvature) {
    if (!options.points) {
        throw new Error('Points file required (--points <file>)');
    }
...
// Helper functions
function parseVector(vectorStr, name) {
    if (!vectorStr) {
...
function projectToPoincareBall(point, curvature) {
...
function displayResult(result, options) {
    ...
    if (result.result && Array.isArray(result.result)) {
        console.log(`\n  Result Vector: [${result.result.slice(0, 5).map((v) => v.toFixed(4)).join(', ')}${result.result.length > 5 ? '...' : ''}]`);
