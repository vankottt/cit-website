...
import { executeMultipleUpdates, rollbackUpdate, getUpdateHistory, clearHistory, } from '../update/executor.js';
    ...
    async action(ctx) {
        ...
        try {
            ...
            if (autoUpdates.length > 0) {
                output.printInfo(`${autoUpdates.length} update(s) will be applied automatically on next startup`);
            }
        ...
        try {
            ...
            const config = {
                ...
                autoUpdate: {
                    patch: true,
                    minor: true,
                    major: flags['include-major'] || false,
                },
            };
            ...
            const updateResults = await executeMultipleUpdates(results, installedPackages, flags['dry-run']);
            ...
            if (successful.length > 0) {
                output.printSuccess(`${successful.length} package(s) ${flags['dry-run'] ? 'would be ' : ''}updated:`);
