...
async function resolveCodexInitializer(cwd) {
...
async function maybeAutoDetectCodex(ctx, options) {
    try {
        ...
        const result = await initializer.initialize({
            projectPath: ctx.cwd,
            template: (options.minimal ? 'minimal' : options.full ? 'full' : 'default'),
            force: options.force,
            dual: false, // Claude Code files were already written by the main init flow above
        });
        if (!result.success)
            return; // best-effort — never fail the primary init over this
        output.writeln();
        output.printBox([
...
function isBloatedRufloSkillDir(dir) {
    try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        if (bytes > BLOAT_BYTES_THRESHOLD)
                            throw new Error('__bloat_threshold__');
                    }
                    catch (err) {
                        if (err instanceof Error && err.message === '__bloat_threshold__')
                            throw err;
...
const initClaudeAction = async (ctx) => {
