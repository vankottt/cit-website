...
const acceptSub = {
    ...
    action: async () => {
        const decision = resolveFunnelEnabled();
        if (!decision.enabled) {
            output.printWarning(`Funnel is currently disabled by: ${decision.decidedBy}. Run 'ruflo funnel enable' first, then re-run accept.`);
            return { success: false, data: decision };
        }
        const current = getDisclosure();
        if (current.state === 'disclosed_disabled') {
            output.printWarning("Disclosure is in a declined state. Run 'ruflo funnel enable' first, then re-run accept.");
            return { success: false, data: current };
        }
        const rec = recordDisclosureAccepted();
        const eligible = promoEligible();
        output.printSuccess(`Disclosure accepted (firstShownAt backdated to ${rec.firstShownAt}). Promo rotation eligible: ${eligible}.`);
...
const openSub = {
    ...
    action: async () => {
        ...
        if (!promo) {
            output.printWarning("No promo has been shown yet. Wait for the statusline to render one, then re-run 'ruflo funnel open'.");
            return { success: false };
        }
        if (!promo.url) {
            output.printWarning(`Current promo (kind=${promo.kind ?? 'unknown'}) has no URL. Nothing to open: "${promo.text}"`);
            return { success: false, data: promo };
        }
        let parsed;
        try {
            parsed = new URL(promo.url);
        }
        catch {
        ...
        if (parsed.protocol !== 'https:' || !OPEN_ALLOWED_HOSTS.has(parsed.hostname)) {
            output.printError(`Refusing to open URL — not on the allowlist: ${parsed.protocol}//${parsed.hostname}`);
