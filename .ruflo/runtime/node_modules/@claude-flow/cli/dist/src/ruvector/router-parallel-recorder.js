...
function rotate(cfg) {
    try {
        ...
        if (process.env.DEBUG) {
            ...
            console.error('router-parallel: rotate failed:', e.message);
        }
    }
}
function appendRow(row) {
    ...
    try {
        ...
        if (process.env.DEBUG) {
            // eslint-disable-next-line no-console
            console.error('router-parallel: appendRow failed:', e.message);
        }
...
export function recordPair(args) {
    ...
    if (!cfg.enabled)
        return { recorded: false, taskHash: hash };
    const row = {
        v: 1,
        type: 'pair',
...
export function __resetParallelRecorderForTests() {
    _cfg = null;
}
//# sourceMappingURL=router-parallel-recorder.js.map