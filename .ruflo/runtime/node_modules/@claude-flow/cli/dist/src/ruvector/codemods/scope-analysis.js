...
function collectIdentifierNames(target, out) {
 ...
 */
export function buildReassignmentResolver(sf) {
        ...
        if (ts.isIdentifier(name)) {
            ...
            for (const el of name.elements) {
                if (ts.isBindingElement(el))
                    addBinding(el.name, scope);
            }
        }
    };
    // Pass 1 — record hoisted names per scope.
    const collectHoisted = (node) => {
    ...
    collectHoisted(sf);
    const resolveScope = (name, from) => {
        let scope = from;
        while (scope) {
            if (hoisted.get(scope)?.has(name))
                return scope;
    ...
    const reassigned = new Set(); // key: `${scopeId}::${name}`
    ...
    const markReassigned = (name, at) => {
        const declScope = resolveScope(name, enclosingScope(at));
        if (declScope)
            reassigned.add(`${scopeId(declScope)}::${name}`);
    };
