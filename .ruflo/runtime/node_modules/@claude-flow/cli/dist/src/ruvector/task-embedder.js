 ...
 */
async function loadEmbedderBatch() {
    const ex = await loadExtractor();
    if (!ex)
        return null;
    return async (texts) => {
 ...
 */
export async function embedTaskWithCache(task) {
 ...
 * accordingly).
 */
export async function embedTaskWithCacheBatch(tasks) {
    const out = new Array(tasks.length).fill(undefined);
...
export function __resetTaskEmbedderForTests() {
    _cache.clear();
    _hits = 0;
    _misses = 0;
    _extractorPromise = null;
}
//# sourceMappingURL=task-embedder.js.map