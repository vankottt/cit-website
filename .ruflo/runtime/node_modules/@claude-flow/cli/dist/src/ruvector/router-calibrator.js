...
export class IsotonicCalibrator {
    ...
    static fit(pairs) {
        ...
        const pools = sorted.map(([p, o]) => ({ predMin: p, predMax: p, sumObs: o, count: 1 }));
    ...
    transform(x) {
        ...
        if (n === 0)
            return x; // No calibration data → pass-through.
        if (n === 1)
            return this.buckets[0].calibrated;
        const mids = this.buckets.map(b => (b.predMin + b.predMax) / 2);
        if (x <= mids[0])
            return this.buckets[0].calibrated;
        if (x >= mids[n - 1])
            return this.buckets[n - 1].calibrated;
    ...
    static fromJSON(j) {
        if (!j || j.v !== 1)
            throw new Error(`unsupported calibrator schema v=${j?.v}`);
...
//# sourceMappingURL=router-calibrator.js.map