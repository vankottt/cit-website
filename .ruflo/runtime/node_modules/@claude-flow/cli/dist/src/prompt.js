...
class PromptManager {
    ...
    async select(options) {
        ...
        // Display options
        const displayChoices = (currentIndex, startIndex = 0) => {
    ...
    async multiSelect(options) {
        const { message, options: choices, default: defaultValues = [], required = false, min, max } = options;
        this.formatter.writeln();
        this.formatter.writeln(this.formatter.bold(`? ${message}`));
    ...
    // Autocomplete Prompt
    // ============================================
    async autocomplete(message, choices, options = {}) {
...
export const select = (options) => promptManager.select(options);
export const confirm = (options) => promptManager.confirm(options);
export const input = (options) => promptManager.input(options);
export const multiSelect = (options) => promptManager.multiSelect(options);
export const text = (message, placeholder) => promptManager.text(message, placeholder);
export const number = (message, options) => promptManager.number(message, options);
export const autocomplete = (message, choices, options) => promptManager.autocomplete(message, choices, options);
