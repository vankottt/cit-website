...
function endsCvc(s) {
    if (s.length < 3)
        return false;
    if (!isCons(s, s.length - 3))
        return false;
    if (isCons(s, s.length - 2))
        return false;
    if (!isCons(s, s.length - 1))
        return false;
...
function replace(word, suffix, replacement, minM) {
 ...
 */
export function luceneTokenize(text) {
    if (!text)
        return [];
    const tokens = [];
    for (const raw of text.toLowerCase().split(/[^a-z0-9]+/g)) {
        if (raw.length < 2)
            continue;
        if (LUCENE_STOPWORDS.has(raw))
            continue;
        if (/^\d+$/.test(raw) && raw.length < 4)
            continue; // drop short numbers
        tokens.push(porterStem(raw));
    }
    return tokens;
}
export function buildLuceneCorpusStats(tokenisedDocs) {
 ...
 */
export function luceneBM25(queryTokens, docTokens, stats, k1 = 1.2, b = 0.75) {
...
//# sourceMappingURL=lucene-bm25.js.map