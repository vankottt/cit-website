 ...
 */
export async function getCrossEncoder(modelName = 'Xenova/ms-marco-MiniLM-L-6-v2') {
    if (singleton)
        return singleton;
    if (loadAttempted)
        return null;
        ...
        singleton = {
            async scoreBatch(query, docs) {
                ...
                for (const doc of docs) {
                    ...
                    const raw = Array.isArray(logits) ? logits : Array.from(logits);
                    let score;
                    if (raw.length === 1) {
 ...
 */
export async function crossEncoderRerank(query, docs, topK) {
...
export function resetCrossEncoder() {
    singleton = null;
    loadAttempted = false;
    loadError = null;
}
//# sourceMappingURL=cross-encoder-rerank.js.map