 ...
 */
export async function runFlywheelWorker(projectRoot, opts = {}) {
    try {
 ...
 */
export async function runFlywheelGenerationWorker(projectRoot, opts = {}) {
    try {
        if (!(opts.optInOverride ?? harnessLoopOptedIn()))
            return { ran: false, reason: 'opt-in required (RUFLO_HARNESS_LOOP=1)', generation: 0 };
        const neural = await import('../mcp-tools/neural-tools.js');
        const tool = neural.neuralTools.find((t) => t.name === 'neural_patterns');
        if (!tool)
...
//# sourceMappingURL=harness-flywheel-runtime.js.map