...
function locateTestgenScripts() {
    ...
    for (const c of candidates) {
        if (existsSync(join(c, 'tdd-repair', 'tdd-repair.mjs')))
            return c;
    }
    return null;
}
 ...
 */
function runTestgenScript(scriptName, args) {
    return new Promise((resolve) => {
        ...
        p.on('close', (code) => {
            ...
            const looksDegraded = !!(json && typeof json === 'object' && json.data?.reason === 'plugin-not-found');
...
const TESTGEN_SUCCESS_SEMANTIC = 'Returns `{success, data, degraded, exitCode}`. success=true when the script ran as designed (including graceful-degradation payloads); ' +
...
export const testgenTools = [
    {
        ...
        description: 'ADR-175-inspired Test-Driven Repair. Use when you have a failing test and want a verified, bounded-cost fix. Spawns a bounded headless `claude -p` (Read/Edit/Bash only, --max-budget-usd capped) ' +
        ...
        handler: async (input) => {
            ...
            if (input.test)
                args.push('--test', String(input.test));
            if (input.testCommand)
                args.push('--test-command', String(input.testCommand));
            if (input.maxAttempts !== undefined)
                args.push('--max-attempts', String(input.maxAttempts));
            if (input.budgetUsd !== undefined)
                args.push('--budget', String(input.budgetUsd));
            if (input.model)
                args.push('--model', String(input.model));
            if (input.confirm === true)
                args.push('--confirm');
            if (input.noTestOracle === true)
                args.push('--no-test-oracle');
...
//# sourceMappingURL=testgen-tools.js.map