...
async function embedTaskSafe(task) {
...
async function determineAgentModel(agentType, config, task) {
    ...
    if (task) {
        try {
            ...
            const routeResult = await enhancedRouter.route(task, { filePath: config.filePath, embedding });
            ...
            const routedBy = routeResult.routedBy === 'hybrid' ? 'hybrid' : 'router';
            return {
            ...
            if (router) {
                try {
                    ...
                    const routedBy = result.routedBy === 'hybrid' ? 'hybrid' : 'router';
                    return {
                        model: result.model,
                        routedBy,
