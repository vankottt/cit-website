...
let graphBackendMod = null;
async function getGraphBackend() {
    if (!graphBackendMod)
        graphBackendMod = await import('../ruvector/graph-backend.js');
        ...
        try {
            ...
            if (mode === 'semantic') {
                try {
                    ...
                    if (!db)
                        return { success: false, error: 'graph_edges DB unavailable (sql.js could not load)', hint: 'Check Node version + try `ruflo memory init` to initialize manually.', mode, nodeId };
                    ...
                    const rows = db.prepare(`SELECT id, source_id, target_id, relation, weight, embedding_ref FROM graph_edges WHERE embedding_ref IS NOT NULL LIMIT ?`).raw().all(budget.maxNodesVisited);
            ...
            if (mode === 'pagerank') {
                try {
                    ...
                    const edges = db.prepare(`SELECT source_id, target_id, weight FROM graph_edges LIMIT ?`).raw().all(budget.maxNodesVisited);
                    if (edges.length === 0) {
...
export const agentdbGraphPathfinder = {
    ...
    handler: async (params) => {
        ...
        try {
            ...
            const db = await getBridgeDb(undefined, { createIfMissing: true });
            if (!db)
                return { success: false, error: 'graph_edges DB unavailable (sql.js could not load)', hint: 'Check Node version + try `ruflo memory init` to initialize manually.', seedNodeId };
            ...
            try {
                const seedHit = db.prepare(`SELECT 1 FROM graph_edges WHERE source_id = ? OR target_id = ? LIMIT 1`).raw().all(seedNodeId, seedNodeId);
                if (seedHit.length === 0) {
            ...
            const rawEdges = db.prepare(`SELECT ${colsSql} FROM graph_edges LIMIT ?`).raw().all(budget.maxNodesVisited);
