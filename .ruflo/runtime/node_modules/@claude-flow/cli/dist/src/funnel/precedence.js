...
function enterprisePolicyDisabled(env) {
    const candidates = [];
    if (env.RUFLO_ENTERPRISE_POLICY)
        candidates.push(env.RUFLO_ENTERPRISE_POLICY);
    if (process.platform === 'win32') {
        if (env.ProgramData)
            candidates.push(path.join(env.ProgramData, 'ruflo', 'policy.json'));
    }
    else {
    ...
    for (const p of candidates) {
        try {
            ...
            if (policy?.funnel?.enabled === false)
                return true;
        }
        catch {
...
function userConfigDisabled() {
    const cfg = readStateJson('funnel.json');
    return cfg?.enabled === false;
}
function projectConfigDisabled(cwd) {
    try {
 ...
 */
function remotePolicyDisabled() {
    const policy = readStateJson('funnel-remote-policy.json');
    return policy?.funnelEnabled === false;
}
export function resolveFunnelEnabled(cwd = process.cwd(), env = process.env) {
    if (envDisabled(env))
        return { enabled: false, decidedBy: 'env' };
    if (enterprisePolicyDisabled(env))
        return { enabled: false, decidedBy: 'enterprise-policy' };
    if (userConfigDisabled())
        return { enabled: false, decidedBy: 'user-config' };
    if (projectConfigDisabled(cwd))
        return { enabled: false, decidedBy: 'project-config' };
    ...
    if (remotePolicyDisabled())
        return { enabled: false, decidedBy: 'remote-policy' };
