...
export class DualModeOrchestrator extends EventEmitter {
    ...
    constructor(config) {
        ...
        this.config = {
            ...
            memoryDbPath: path.resolve(config.memoryDbPath
    ...
    async initializeSharedMemory(taskContext) {
        ...
        await this.runCommand('npx', ['ruflo@latest', 'memory', 'init'], projectPath);
        // Store task context
    ...
    async executeHeadless(config) {
        ...
        return new Promise((resolve, reject) => {
            ...
            proc.stderr?.on('data', (data) => {
                if (errorOutput.length < maxOutputBytes)
                    errorOutput += data.toString().slice(0, maxOutputBytes - errorOutput.length);
            });
...
Remember: Other agents depend on your results in shared memory. Be concise and store actionable outputs.`;
            ...
            for (const batch of this.partitionLevel(level)) {
                await Promise.all(batch.map(async (worker) => {
                    ...
                    if (result?.status === 'failed')
                        errors.push(`${worker.id}: ${result.error}`);
                }));
        ...
        for (const worker of workers) {
            for (const dependency of worker.dependsOn ?? []) {
                if (!ids.has(dependency))
                    throw new Error(`worker ${worker.id} depends on missing worker ${dependency}`);
            }
        }
        const levels = [];
    ...
    async collectSharedMemory() {
        ...
        try {
            const output = await this.runCommand('npx', ['ruflo@latest', 'memory', 'list', '--namespace', sharedNamespace, '--format', 'json'], projectPath);
    ...
    async authorizeWorker(worker) {
        ...
        const raw = await this.runCommand('npx', ['ruflo@latest', 'policy', 'evaluate', JSON.stringify(request)], this.config.projectPath);
    ...
    resolveWorkerEnvelope(worker) {
        ...
        if (!valid)
            throw new Error(`worker ${worker.id} capability envelope cannot expand`);
        return child;
    }
    workerEnvironment(worker) {
        ...
        env.CLAUDE_FLOW_CAPABILITY_ENVELOPE = JSON.stringify(this.resolveWorkerEnvelope(worker));
        return env;
    }
...
export function loadSwarmAutomationConfig(projectPath) {
    const defaults = {
        enabled: false,
        maxConcurrent: 4,
        maxWriters: 2,
