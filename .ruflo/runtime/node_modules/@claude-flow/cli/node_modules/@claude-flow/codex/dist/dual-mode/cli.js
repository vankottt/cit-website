...
import { DualModeOrchestrator, CollaborationTemplates, loadSwarmAutomationConfig, } from './orchestrator.js';
import { CodexWorktreeCoordinator } from '../worktrees/coordinator.js';
/**
 * Create the dual-mode command
 */
export function createDualModeCommand() {
...
function createRunCommand() {
    return new Command('run')
        ...
        .option('-w, --worker <spec>', 'Worker spec "<platform>:<role>:<prompt>" (platform = claude|codex). Repeatable. Workers chain sequentially unless --parallel-workers.', (val, acc) => { acc.push(val); return acc; }, [])
        ...
        let workers;
        let taskContext;
        const workerSpecs = options.worker ?? [];
        const templateName = options.template ?? templateArg;
        ...
        if (automation.worktreeIsolation) {
            ...
            const record = coordinator.prepare(runId, workers.map((worker) => ({
                id: worker.id,
...
export async function loadWorkerConfig(configPath, cwd = process.cwd()) {
 ...
 */
export function parseWorkerSpecs(specs, parallel) {
    ...
    specs.forEach((spec, index) => {
        ...
        usedIds.add(id);
        const worker = { id, platform, role, prompt };
 ...
 */
function getTemplateWorkers(template, task) {
    switch (template) {
        case 'feature':
