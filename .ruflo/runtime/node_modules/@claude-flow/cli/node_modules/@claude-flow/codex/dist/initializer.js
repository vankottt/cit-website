...
import { BUILT_IN_SKILL_NAMES, generateSkillMd, generateBuiltInSkill, } from './generators/skill-md.js';
...
export class CodexInitializer {
    ...
    async initialize(options) {
        ...
        try {
            ...
            if (options.skills === undefined) {
                ...
                if (omitted.length > 0) {
                    warnings.push(`Omitted ${omitted.length} catalog skills without canonical packaged assets. ` +
     ...
     */
    async retainCanonicalPackagedSkills() {
...
`;
    ...
    async updateGitignore() {
        ...
        const missing = GITIGNORE_ENTRIES.filter((entry) => entry.length > 0 && !existingLines.has(entry));
        if (missing.length === 0)
            return false;
