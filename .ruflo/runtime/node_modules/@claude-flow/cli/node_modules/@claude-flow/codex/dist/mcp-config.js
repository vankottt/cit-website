...
export function getCodexCliInvocation(lookupOutput, platform = process.platform, commandShell = process.env.ComSpec || 'cmd.exe') {
...
export function getRufloMcpServerConfig(platform = process.platform, toolTimeout = 120) {
...
export function renderMcpServerToml(server) {
    const lines = [
...
export function getRufloMcpAddCommand(platform = process.platform) {
    const server = getRufloMcpServerConfig(platform);
    return ['codex', 'mcp', 'add', RUFLO_MCP_SERVER_NAME, '--', server.command, ...(server.args ?? [])].join(' ');
}
export function hasExpectedRufloMcpTransport(registration, platform = process.platform) {
...
export function hasExpectedRufloMcpTimeout(registration) {
    return typeof registration.startup_timeout_sec === 'number'
        && registration.startup_timeout_sec >= RUFLO_MCP_STARTUP_TIMEOUT_SEC;
}
export function upsertMcpServerStartupTimeout(config, serverName = RUFLO_MCP_SERVER_NAME, timeoutSec = RUFLO_MCP_STARTUP_TIMEOUT_SEC) {
