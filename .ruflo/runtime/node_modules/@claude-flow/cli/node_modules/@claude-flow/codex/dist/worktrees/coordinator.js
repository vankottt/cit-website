...
export class CodexWorktreeCoordinator {
    ...
    constructor(repoRoot) {
        ...
        if (resolve(top) !== this.repoRoot)
            throw new Error(`repoRoot must be the git top-level: ${top}`);
    ...
    prepare(runId, agents, options = {}) {
        ...
        try {
            ...
            for (const assignment of assignments.reverse()) {
                ...
                catch { /* retain work for recovery */ }
            }
            throw error;
        }
    }
    ...
    integrate(runId, agentIds) {
        const record = this.status(runId);
        const wanted = agentIds ? new Set(agentIds) : null;
        const merged = [];
        for (const assignment of record.assignments) {
            if (assignment.readOnly || (wanted && !wanted.has(assignment.agentId)))
                continue;
    ...
    cleanup(runId) {
        ...
        if (retained.length === 0)
            unlinkSync(this.registryPath(runId));
