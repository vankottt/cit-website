 ...
 * a validation call.
 */
async function canonicalize(resolvedPath) {
    try {
 ...
 */
function canonicalizeSync(resolvedPath) {
    try {
 ...
 */
function isWithinPrefix(resolvedPath, prefix) {
...
export class PathValidator {
    ...
    constructor(config) {
        ...
        this.canonicalPrefixes = this.resolvedPrefixes.map(p => this.config.resolveSymlinks ? canonicalizeSync(p) : p);
    }
    /**
    ...
    addPrefix(prefix) {
        ...
        if (!this.resolvedPrefixes.includes(resolved)) {
            ...
            this.canonicalPrefixes.push(this.config.resolveSymlinks ? canonicalizeSync(resolved) : resolved);
        }
    }
    /**
    ...
    isWithinAllowed(inputPath) {
        try {
            ...
            return this.resolvedPrefixes.some(prefix => isWithinPrefix(resolved, prefix));
        }
        catch {
            return false;
        }
