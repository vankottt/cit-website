...
export function createMcpTools(getCoordinator, getContext, getWgMesh = () => null) {
    ...
    function requireWgMesh() {
        const w = getWgMesh();
        if (!w)
            throw new Error('WG mesh layer not initialized (set config.wgMesh = true and inject WgMeshService)');
        return w;
    }
    return [
        {
            ...
            handler: async (params) => {
                ...
                const nodeId = params['nodeId'] ?? `node-${Date.now().toString(36)}`;
                await coordinator.initialize({
                    nodeId,
                    publicKey: '',
                    endpoint: params['endpoint'],
                    capabilities: {
                        agentTypes: params['agentTypes'] ?? ['coder', 'reviewer'],
        ...
        },
        {
            ...
            handler: async (params) => {
                const coordinator = requireCoordinator();
                const session = await coordinator.joinPeer(params['endpoint'], params['manifest']);
        ...
        },
        {
            ...
            handler: async (params) => {
                const coordinator = requireCoordinator();
                const result = await coordinator.sendMessage(params['targetNodeId'], params['messageType'], params['payload'], {
        ...
        },
        {
            ...
            handler: async (params) => {
                const coordinator = requireCoordinator();
                const result = await coordinator.sendMessage(params['targetNodeId'], 'memory-query', { query: params['query'], namespace: params['namespace'] ?? 'default' });
                return textResult(JSON.stringify(result, null, 2), !result.success);
            },
        },
        {
        ...
        },
        {
            ...
            handler: async (params) => {
                ...
                if (!context)
                    return textResult('Plugin not initialized', true);
                const audit = context.services.get('federation:audit');
                if (!audit)
                    return textResult('Audit service not found', true);
        ...
        },
        {
            ...
            handler: async (params) => {
                const coordinator = requireCoordinator();
                const ok = await coordinator.evictPeer(params['nodeId'], 'MANUAL_EVICT', params['correlationId']);
                const msg = ok
        ...
        },
        {
            ...
            handler: async (params) => {
                const coordinator = requireCoordinator();
                const ok = await coordinator.reactivatePeer(params['nodeId'], params['correlationId']);
                const msg = ok
        ...
        },
        {
            ...
            handler: async (params) => {
                ...
                if (!context)
                    return textResult('Plugin not initialized', true);
                const routing = context.services.get('federation:routing');
                if (!routing)
                    return textResult('Routing service not found', true);
                const proposal = await routing.propose(params['messageType'], params['payload'], params['quorumFraction'] ?? 2 / 3);
        ...
        },
        {
            ...
            handler: async () => {
                ...
                return textResult('federation_wg_attest: wire WgWitnessService in plugin.ts (Phase 6 follow-up). Until then, use the WgWitnessService class directly from the federation package.', true);
            },
        },
        {
