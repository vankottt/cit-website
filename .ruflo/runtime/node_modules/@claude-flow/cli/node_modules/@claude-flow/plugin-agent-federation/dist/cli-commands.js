...
function requireCoordinator(get) {
    const c = get();
    if (!c)
        throw new Error('Federation not initialized. Run "federation init" first.');
    return c;
}
function readPeerManifest(value) {
    if (value === undefined)
        return undefined;
...
export function createCliCommands(getCoordinator, getContext) {
    return [
        {
            ...
            handler: async (args) => {
                ...
                const nodeId = args['node-id'] ?? `node-${Date.now().toString(36)}`;
                await coordinator.initialize({
                    ...
                    capabilities: {
                        ...
                        complianceModes: [args['compliance']],
                    },
        ...
        },
        {
            ...
            handler: async (args) => {
                const coordinator = requireCoordinator(getCoordinator);
                const session = await coordinator.joinPeer(args._[0], readPeerManifest(args['manifest']));
            ...
            handler: async (args) => {
                ...
                if (format === 'json') {
                    ...
                    for (const [nodeId, level] of Object.entries(status.trustLevels)) {
                        console.log(`${nodeId.padEnd(33)}| ${getTrustLevelLabel(level)}`);
                    }
                }
            },
        },
        ...
        },
        {
            ...
            handler: async (args) => {
                ...
                if (exportFormat) {
                    const output = await audit.export(query, exportFormat);
                    console.log(output);
                }
                else {
        ...
        },
        {
            ...
            description: 'ADR-164 §3.5.4 — Founder-bootstrap trust elevation escape hatch. ' +
            ...
            handler: async (args) => {
                ...
                if (auditFlag !== true && auditFlag !== 'true') {
                    console.error('--audit is mandatory for bootstrap elevation (ADR-164 §3.5.4). ' +
                ...
                switch (toStr) {
                    ...
                    case 'PRIVILEGED':
                    case '4':
                        target = TrustLevel.PRIVILEGED;
                        break;
                }
                if (target === null) {
                    console.error(`--to value "${rawTo}" not recognized. ` +
                ...
                if (entry === null) {
                    console.error(`Node "${nodeId}" is not a registered federation peer. ` +
                        `Run "ruflo-federation peers add <endpoint>" first.`);
                    process.exitCode = 1;
                    return;
                }
                // --audit is mandatory; always print the entry to stdout.
                const human = `[BOOTSTRAP-ELEVATION] ${entry.timestamp}\n` +
                    `  nodeId:         ${entry.nodeId}\n` +
        ...
        },
        {
            ...
            handler: async (args) => {
                ...
                console.log(`Trust Level: ${getTrustLevelLabel(trustLevel)} (${trustLevel})`);
            },
        },
        {
