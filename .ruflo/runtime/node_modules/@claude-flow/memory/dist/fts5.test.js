...
async function newSqlJs() {
    if (!WASM_PATH)
        throw new Error('sql.js/dist/sql-wasm.wasm not found');
...
describe('Phase 5 — SqlJsBackend FTS5', () => {
    it('searchKeyword returns matching entries for an FTS phrase', async () => {
        ...
        const results = await backend.searchKeyword('authentication patterns', 10);
    ...
    it('searchKeyword does not return a deleted entry', async () => {
        ...
        await backend.store(entry);
        let results = await backend.searchKeyword('unique-marker-phrase', 5);
        expect(results.length).toBe(1);
        await backend.delete(entry.id);
        results = await backend.searchKeyword('unique-marker-phrase', 5);
    ...
    it('searchKeyword reflects content updates', async () => {
        ...
        await backend.store(entry);
        let results = await backend.searchKeyword('apples', 5);
        expect(results.length).toBe(1);
        await backend.update(entry.id, { content: 'rewritten phrase about bananas' });
        results = await backend.searchKeyword('apples', 5);
        expect(results.length).toBe(0);
        results = await backend.searchKeyword('bananas', 5);
        expect(results.length).toBe(1);
        await backend.shutdown();
    });
    it('searchKeyword returns [] for an empty query', async () => {
        const backend = await newSqlJs();
        const results = await backend.searchKeyword('   ', 5);
...
describe('Phase 5 — SQLiteBackend FTS5 (better-sqlite3)', async () => {
    ...
    maybeIt('searchKeyword returns matching entries with FTS5 ranking', async () => {
        ...
        const results = await backend.searchKeyword('authentication', 10);
        expect(results.length).toBeGreaterThan(0);
        for (const r of results) {
    ...
    maybeIt('searchKeyword stops finding entries after delete', async () => {
        ...
        await backend.store(entry);
        let results = await backend.searchKeyword('unique-marker', 5);
        expect(results.length).toBe(1);
        await backend.delete(entry.id);
        results = await backend.searchKeyword('unique-marker', 5);
        expect(results.length).toBe(0);
        await backend.shutdown();
    });
});
