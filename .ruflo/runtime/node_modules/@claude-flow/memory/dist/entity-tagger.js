...
export function extractEntities(text) {
    ...
    for (const m of text.matchAll(/(?:[A-Za-z]:[\\/])?(?:[\w.-]+[\\/])+[\w-]+\.\w{1,5}\b/g)) {
        add(m[0]);
    }
    for (const m of text.matchAll(/(?:\.{1,2}|[A-Za-z]:)[\\/][\w./\\-]+/g))
        add(m[0]);
    ...
    for (const m of text.matchAll(/(?<!\w)"([^"]{2,})"(?!\w)/g))
        add(m[1]);
    for (const m of text.matchAll(/(?<!\w)'([^']{2,})'(?!\w)/g))
        add(m[1]);
...
//# sourceMappingURL=entity-tagger.js.map