...
function isHybridCapableBackend(backend) {
...
export class ControllerRegistry extends EventEmitter {
     ...
     */
    async closePriorIfAny(name) {
        const prior = this.controllers.get(name);
        if (!prior?.instance)
            return;
        const inst = prior.instance;
        try {
    ...
    async createController(name) {
        switch (name) {
            ...
            case 'hybridSearch': {
                ...
                const directBackend = !memSvc && isHybridCapableBackend(this.backend) ? this.backend : null;
                ...
                return {
                    ...
                    search: async (query, opts = {}) => {
                        ...
                        const toCands = (results) => results.map((r) => ({
                            id: r.entry.id,
                            key: r.entry.key,
                        ...
                        const [dense, sparse, entityHits] = await Promise.all([
                            adapter.semanticSearch(query, fanOutK).catch(() => []),
                            adapter.searchKeyword(query, { k: fanOutK }).catch(() => []),
                            entities.length > 0
                                ? Promise.all(entities.map((e) => adapter.searchKeyword(e, { k: entityFanOut }).catch(() => []))).then((perEntity) => perEntity.flat())
                                : Promise.resolve([]),
                        ]);
                        ...
                        const candKey = (c) => c.id || c.key || c.content.slice(0, 128);
                        ...
                        if (entityCands.length > 0)
                            arms.push(entityCands);
            ...
            case 'hierarchicalMemory': {
                ...
                return this.tieredMemoryFallback(this.agentdb ? 'agentdb-export-missing' : 'agentdb-unavailable');
            }
            case 'memoryConsolidation': {
