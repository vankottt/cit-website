...
export class AgentDBAdapter extends EventEmitter {
    ...
    async semanticSearch(content, k = 10, threshold) {
        if (!this.config.embeddingGenerator) {
            ...
            return this.searchKeyword(content, { k, threshold });
        }
        try {
     ...
     */
    async searchKeyword(query, options = { k: 10 }) {
    ...
    async loadFromDisk() {
        ...
        try {
            hnswExists = fs.existsSync(hnswPath);
        }
        catch { /* ignore */ }
        try {
     ...
     */
    buildPersistedMeta() {
        const entriesArr = [...this.entries.values()].sort((a, b) => a.id.localeCompare(b.id));
