...
function writeSourceFile(entries) {
...
const embeddingOf = (content) => new Float32Array([content.length, 1, 2]);
describe('MemoryMigrator — batch embedding', () => {
    it('uses the batch embedding generator ONCE per batch (not per entry)', async () => {
        ...
        const batchCalls = [];
        const batchGenerator = async (contents) => {
            batchCalls.push(contents);
            return contents.map(embeddingOf);
        };
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, batchSize: 10 }, undefined, batchGenerator);
    ...
    it('falls back to bounded concurrency over the single-text generator', async () => {
        ...
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, batchSize: 20, embeddingConcurrency: 4 }, generator);
        const result = await migrator.migrate();
    ...
    it('one failing embedding does not fail the batch — entry stored without vector', async () => {
        ...
        const generator = async (content) => {
            if (content === 'POISON')
                throw new Error('embedder choked');
            return embeddingOf(content);
        };
        const warnings = [];
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, batchSize: 10 }, generator);
        migrator.on('migration:warning', (w) => warnings.push(w.message));
        const result = await migrator.migrate();
    ...
    it('falls back to single-text embedding when the batch call fails', async () => {
        ...
        const singleGenerator = vi.fn(async (content) => embeddingOf(content));
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, batchSize: 10 }, singleGenerator, batchGenerator);
        const result = await migrator.migrate();
    ...
    it('preserves validation-skip semantics (invalid entries never reach the embedder)', async () => {
        ...
        const batchGenerator = vi.fn(async (contents) => contents.map(embeddingOf));
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, batchSize: 10 }, undefined, batchGenerator);
        const result = await migrator.migrate();
    ...
    it('skips embedding entirely when generateEmbeddings=false', async () => {
        ...
        const { stored, target } = makeTarget();
        const batchGenerator = vi.fn(async (contents) => contents.map(embeddingOf));
        const migrator = new MemoryMigrator(target, { source: 'json', sourcePath, generateEmbeddings: false }, undefined, batchGenerator);
        const result = await migrator.migrate();
        expect(result.progress.migrated).toBe(1);
