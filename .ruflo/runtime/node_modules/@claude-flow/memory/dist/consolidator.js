...
export class MemoryConsolidator {
     ...
     */
    async sweepExpired() {
        const adapter = this.service.getAdapter();
        const entries = adapter.entries;
        const namespaceIndex = adapter.namespaceIndex;
        const keyIndex = adapter.keyIndex;
        const tagIndex = adapter.tagIndex;
    ...
    async dedup(strategy) {
        ...
        for (const bucket of buckets.values()) {
            ...
            if (effective === 'keep-oldest') {
                keeper = bucket.reduce((acc, e) => e.createdAt < acc.createdAt ? e : acc);
            }
            else {
                // keep-newest + merge-tags share this branch
                keeper = bucket.reduce((acc, e) => e.updatedAt > acc.updatedAt ? e : acc);
            }
    ...
    async compactHnsw() {
        ...
        if (typeof adapter.emit === 'function') {
            fresh.on('point:added', (data) => adapter.emit('index:added', data));
        }
