...
const UNIFFI_FOREIGN_FUTURE_HANDLE_MAP = new handle_map_ts_1.UniffiHandleMap();
// Some degenerate functions used for default arguments.
const notExpectedError = (err) => false;
function emptyLowerError(e, _alloc) {
    throw new Error("Unreachable");
}
function uniffiTraitInterfaceCallAsync(makeCall, handleSuccess, handleError, lowerString, alloc) {
    return uniffiTraitInterfaceCallAsyncWithError(makeCall, handleSuccess, handleError, notExpectedError, emptyLowerError, lowerString, alloc);
}
function uniffiTraitInterfaceCallAsyncWithError(makeCall, handleSuccess, handleError, isErrorType, lowerError, lowerString, alloc) {
    ...
    const promise = makeCall(abortController.signal)
        ...
        if (isErrorType(error)) {
            try {
                handleError(rust_call_ts_1.CALL_ERROR, lowerError(error, alloc));
                return;
            }
            catch (e) {
        ...
        handleError(rust_call_ts_1.CALL_UNEXPECTED_ERROR, lowerString(message, alloc));
    });
...
//# sourceMappingURL=async-callbacks.js.map