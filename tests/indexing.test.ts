...
describe("indexing policy", () => {
  it("is noindex unless CIT_ALLOW_INDEXING is true", () => {
    expect(allowPublicIndexing({})).toBe(false);
    expect(allowPublicIndexing({ CIT_ALLOW_INDEXING: "true" })).toBe(true);
    expect(allowPublicIndexing({ CIT_ALLOW_INDEXING: "true", VERCEL_ENV: "preview" })).toBe(false);
    expect(allowPublicIndexing({ CIT_ALLOW_INDEXING: "true", VERCEL_ENV: "production" })).toBe(true);
  });
